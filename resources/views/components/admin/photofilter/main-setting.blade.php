<x-admin.card.normal title="{{__('admin/config/upFilter.form_main_setting')}}">
  ...
  <div class="row">
    ...
    <x-admin.form.select-arr name="type" sendvalue="{{old('type',$row->type)}}" :send-arr="$filterTypeArr"
        ...
        @foreach($rowDataSize as $DataSize)
          <tr>
            ...
            @can($PrefixRole.'_edit')
              <td class="td_action">
                <x-admin.form.action-button url="{{route('config.upFilter.size.edit',$DataSize->id)}}" type="edit" :tip="true"/>
              </td>
            @endcan
            @can($PrefixRole.'_delete')
              <td class="td_action">
                <x-admin.form.action-button id="{{route('config.upFilter.size.destroy',$DataSize->id)}}" type="deleteSweet" url="#"/>
              </td>
            @endcan
          </tr>
        @endforeach
...
@can($PrefixRole.'_add')
  @if(intval($row->id)!= '0' and  count($rowDataSize) < 2 )
    <hr>
    <x-admin.form.action-button url="{{route('config.upFilter.size.create',$row->id)}}"
                                print-lable="{{__('admin/config/upFilter.form_add_new_size')}}" size="m" :tip="false"/>
    <hr>
  @endif
@endcan
