<div class="col-lg-{{$col}}">
  ...
  @if($addFilterList)
    @if(intval(issetArr($modelSettings,$controllerName.$filterName,0)) == 0)
      <x-admin.form.select-arr :label="__('admin/config/upFilter.form_select_filter_lable')" :name="$filterInputName" col="8"
      ...
      @if(isset($row->$dbName) and $row->$dbName != '')
        ...
        @if($remove )
          <div class="row mt-3 mr-2 ml-2">
            <x-admin.form.action-button url="{{route($PrefixRoute.$route,$row->id)}}" type="delete" :tip="true"/>
          </div>
        @endif
      @endif
    @endif
