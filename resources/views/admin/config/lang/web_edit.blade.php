          ...
          <form action="{{route('weblang.updateFile')}}" method="post">
            ...
            @foreach($mergeData as $key=>$val)
              <div class="row {{isSetKeyForLang($key)}}">
                ...
                @foreach(config('app.web_lang') as $langkey=>$lang )
                  <div class="col-4">
                    <textarea name="{{$langkey}}[]"
                              class="form-control @if($langkey == 'en') dir_en @endif">{!! str_replace("&amp;lt;br&amp;gt;","\n", AdminHelper::arrIsset($allData[$langkey],$key,""))  !!}</textarea>
                  </div>
                @endforeach
...
@push('JsCode')
  ...
  <script>
      $('#selectfile').change(function () {
          ...
          window.location = "{{ route('weblang.edit','id=') }}" + idSel;
      });
  </script>
@endpush
